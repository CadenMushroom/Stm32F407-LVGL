...
/*******************************/
#define ILI9806G_LESS_PIXEL	480
#define ILI9806G_MORE_PIXEL	800
// 4.5寸屏GT5688驱动配置
...
uint8_t capIftouch(void)
{
    ...
    if((finger & 0x80) == 0)//判断buffer status位
    {
		...
		if((finger & 0x80) == 1)//判断buffer status位
    {
...
/**
  ...
  * @retval 无
  */
static void Goodix_TS_Work_Func(int16_t* cx , int16_t* cy)
{
...
//								HAL_UART_Transmit(&huart1, &xh, 1, 0xFFFF);
//								HAL_UART_Transmit(&huart1, &xl, 1, 0xFFFF);
...
//检测到触摸中断时调用，在中断里
void GTP_TouchProcess(int16_t* cx, int16_t* cy)
{
   
  Goodix_TS_Work_Func(cx, cy);
}
